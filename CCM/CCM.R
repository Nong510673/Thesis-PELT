### this code was modified from code provided in scientific research entitled, "Causal networks of phytoplankton diversity and biomass are modulated by environmental context"
...
setwd("C:/Users/User/Desktop/CCM_V5/Overall") 
seed=25647
set.seed(seed)
...
nomz=function(x, normalization=T, dseason=F, season_sd=F, sea=12, dtrend=T, dTtype="linear"){
  x=as.numeric(x)
  xt=x
  # Detrend
...
month.dat=read.csv("UASB_1003_V3.csv",header=T,fill=T)
n=nrow(month.dat) # time series length
vb=c('date','day','COD',"Fr",'Ac','Pr','Bu','H2','PC1','PC2',"PC3",'richness','evenness','shannon','IS_PN','IP_PN','Cbet','Instability','complexity')# rename the variables
...
# Detrend
#sdat=data.frame(apply(month.dat[,-c(1:2)],2,nomz))

# #The index for testing causal links (a total of 12 links)
# indmat=matrix(0,128,2);colnames(indmat)=c('Effect','Cause')
# indmat[,1]=c(rep('COD', each = 8),rep('dG', each = 8),rep('Ac', each = 8),rep('Pr', each = 8),rep('Bu', each = 8),rep('H2', each = 8),
#              rep('PC1', each = 10),rep('PC2', each = 10),rep('alpha_diversity', each = 10),rep('Biomass', each = 10),
#              rep('Instability', each = 10),rep('positive_proportion', each = 10),rep('negative_proportion', each = 10),rep('interaction_Complexity', each = 10))
# indmat[,2]=c(rep(c('PC1','PC2','alpha_diversity','Biomass','Instability','positive_proportion','negative_proportion','interaction_Complexity'), times = 6),
#              rep(c('COD','dG','Ac','Pr','Bu','H2','Instability','positive_proportion','negative_proportion','interaction_Complexity'), times = 4),
#              rep(c('COD','dG','Ac','Pr','Bu','H2','PC1','PC2','alpha_diversity','Biomass'), times = 4))
# indmat

indmat=matrix(0,17*17,2);colnames(indmat)=c('Effect','Cause')
indmat[,1]= rep(c('COD',"Fr",'Ac','Pr','Bu','H2','PC1','PC2',"PC3",'richness','evenness','shannon','IS_PN','IP_PN','Cbet','Instability','complexity'), each = 17)
indmat[,2]= rep(c('COD',"Fr",'Ac','Pr','Bu','H2','PC1','PC2',"PC3",'richness','evenness','shannon','IS_PN','IP_PN','Cbet','Instability','complexity'), times = 17)
indmat
...
lib_siz=sort(c(5,10,20,30,40,seq(50,n,50),n))
#seq(50,n,50),n))
#seq(50,n,50),n)) # a sequence of library size
...
uncertain.i=data.frame(system=rep('UASB',nrow(indmat)),indmat,uncertain.i)
Convergence=ccmda$p_Z<0.05 & ccmda$Kendall_p<=0.05 & ccmda$Kendall_tau>0
ccmda=data.frame(ccmda,Convergence)

#Lable factor type
indmat_type=matrix(0,17*17,2);colnames(indmat)=c('Effect_type','Cause_type')
indmat_type[,1]= rep(c('Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Community','Community','Community','Community','Community','Community','interaction','interaction','interaction','interaction','interaction'), each = 17)
indmat_type[,2]= rep(c('Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Community','Community','Community','Community','Community','Community','interaction','interaction','interaction','interaction','interaction'), times = 17)
indmat_type



#'Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Metabolites','Community','Community','Community','Community','Community','Community','Community','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Interaction','Community','Other','Other'




# Standardized linkage strength by dividing the maximal linkage strength within the a system
(linkM.std=data.frame(system=rep('UASB',nrow(indmat)),ccmda[,c('Cause','Effect','rho_Lmax','p_Z','Kendall_tau','Kendall_p','Convergence')],
                      ...
                      Cause_type=indmat_type[,1],Effect_type=indmat_type[,2]))
write.csv(linkM.std,'ccmda_UASB_dtrend_T_dTtype_linear_v6_all_combination_ALL.csv',row.names=F)
